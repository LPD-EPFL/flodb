...
#define DS_CONTAINS(s,k,t)        sl_herlihy_contains(s, k)
#define DS_ADD(s,k,t)             sl_herlihy_add(s, k, k, 0)
#define DS_REMOVE(s,k,t)          sl_herlihy_remove(s, k)
#define DS_SIZE(s)                sl_set_size(s)
#define DS_NEW()                  sl_set_new()
#define DS_MULTI_INSERT(s,k,v,seq,n,rep)    sl_herlihy_multi_insert(s,k,v,seq,n,rep)

#define DS_TYPE                   sl_intset_t
#define DS_NODE                   sl_node_t
#define NUMKEYS_MULTIINSERT       500
...
int multiinsert_only = 0;
int multiinsert_seq = 0;
__thread ALIGNED(CACHE_LINE_SIZE) uint64_t key_comps;
__thread ALIGNED(CACHE_LINE_SIZE) uint64_t boosts;
...
volatile ticks *multiinsert_boosts_count;
volatile ticks *multiinsert_key_comparisons_count;
...
test(void* thread) 
{
  ...
  for(i = 0; i < num_elems_thread; i++) 
    {
      ...
      // add function returns old value 
      // if old value != 0, something else existed already there
      if(DS_ADD(set, key, NULL) != 0)
    {
      i--;
    }
    }
  ...
  while (stop == 0) 
    {
      ...
      if (multiinsert_only > 0) {

          //multiinsert
          size_t numkeys = multiinsert_only;
          uint64_t keys[numkeys];
          uint64_t vals[numkeys];
          uint64_t seqs[numkeys];
          uint64_t replaced[numkeys];
          keys[0] = key;
          vals[0] = key;
          seqs[0] = 0;
          ...
          if (multiinsert_seq){
            for ( i = 1; i < numkeys; i++ ){  
              //generate neighboring key
              c = (uint32_t)(my_random(&(seeds[0]),&(seeds[1]),&(seeds[2])));  
              c = (c % (2*multiinsert_seq)) - multiinsert_seq;     
            ...
            for ( i = 1; i < numkeys; i++ ){
              ...
              vals[i] = (c & rand_max) + rand_min;
              seqs[i] = 0;  

            }    
          }

          my_removing_count_succ += DS_MULTI_INSERT(set, keys, vals, seqs,numkeys, replaced);        
          my_removing_count += numkeys;   
      } else {
          int res;                
          START_TS(1);              
  ...
  multiinsert_boosts_count[ID] += boosts;
  multiinsert_key_comparisons_count[ID] += key_comps;
...
main(int argc, char **argv) 
{
  ...
  while(1) 
    {
      i = 0;
      c = getopt_long(argc, argv, "hAf:d:i:n:r:s:u:m:a:l:p:b:v:f:m:", long_options, &i);
    ...
    case 'r':
      ...
      break;
  case 'm':
    multiinsert_only = atoi(optarg); //number of keys inserted in a multiinsert operation
    break;
  case 's':
    multiinsert_seq = atoi(optarg); //size of neighborhood for extracted keys for multiinsert
    break;
    case 'u':
      update = atoi(optarg);
      break;
  ...
  multiinsert_boosts_count = (ticks *) calloc(num_threads , sizeof(ticks));
  multiinsert_key_comparisons_count = (ticks *) calloc(num_threads , sizeof(ticks));
  ...
  volatile uint64_t key_comps_total = 0;
  volatile uint64_t boosts_total = 0;
  ...
  for(t=0; t < num_threads; t++) 
    {
      ...
      key_comps_total += multiinsert_key_comparisons_count[t];
      boosts_total += multiinsert_boosts_count[t];

...
int herlihy_find(sl_intset_t* set, uint64_t key, sl_node_t** preds, sl_node_t** succs);
...
uint64_t herlihy_add(sl_intset_t* set, uint64_t key, uint64_t val, uint64_t seq)
{
    ...
    sl_node_t* new_element = sl_new_simple_node(key, topLevel, 0);
    ...
    while(1) {
        if(herlihy_find(set, key, preds, succs)) {
            ...
            if(CAS_PTR_bool(&preds[0]->next[0], succs[0], new_element)) {
                sl_add_bytes(sizeof(sl_node_t) + (new_element->toplevel + 1) * sizeof(sl_node_t*) +  leveldb::kBigValueSize);
...
uint64_t herlihy_remove(sl_intset_t* set, uint64_t key) {
    ...
    while(1) {
        if(!herlihy_find(set, key, preds, succs)){
            ...
            uint64_t old_value = node_to_remove->val;
...
int herlihy_find_from_preds(sl_intset_t* set, uint64_t key, sl_node_t** old_preds, sl_node_t** preds, sl_node_t** succs) {
    sl_node_t* pred = NULL;
    sl_node_t* curr = NULL;
    sl_node_t* succ = NULL;
    uint32_t level = 0;
...
int multi_insert(sl_intset_t* set, uint64_t* keys, uint64_t* values, 
    ...
    for (i = 0; i < nkeys; i++) {
        ...
        sl_node_t* new_element = sl_new_simple_node(keys_vals_seqs[i].key, topLevel, 0);
        new_element->val = keys_vals_seqs[i].val;
        new_element->seq = keys_vals_seqs[i].seq;

        // if (new_element->val == 0 || new_element->seq == 0){
        //     printf("Val or SEQ == 0; shouldn't happen!\n");
        // }

        while(1) {
            if(herlihy_find_from_preds(set, keys_vals_seqs[i].key, running_preds, preds, succs)) {

                // printf("Found %d; not inserting\n", keys[i]);

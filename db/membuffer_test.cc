...
TEST(MembufferTest, AlwaysCorrect) {
    ASSERT_TRUE(true);
}

TEST(MembufferTest, CreateAndDestroy) {
    MemBuffer* m = new MemBuffer();
    delete m;
}

TEST(MembufferTest, Empty) {
    MemBuffer* m = new MemBuffer();
    Slice key("OANA");
    std::string val;
    Status s;
...
TEST(MembufferTest, Add) {
    MemBuffer* m = new MemBuffer();
    Slice key("OANA");
    std::string val;

    m->Add(kTypeValue, key, key);
...
TEST(MembufferTest, AddAdd) {
    MemBuffer* m = new MemBuffer();
    Slice oana("OANA");
    Slice igor("IGOR");
    std::string val;

    m->Add(kTypeValue, oana, oana);

    Status s;
    LookupKey lkey(oana, 0);
    m->Get(lkey, &val, &s);
    ASSERT_TRUE(s.ok());
    ASSERT_EQ("OANA", val);

    m->Add(kTypeValue, oana, igor);
    m->Get(lkey, &val, &s);
    ASSERT_TRUE(s.ok());
    ASSERT_EQ("IGOR", val);
...
TEST(MembufferTest, AddDeleteAdd) {
    MemBuffer* m = new MemBuffer();
    Slice oana("OANA");
    Slice igor("IGOR");
    std::string val;
    ...
    m->Add(kTypeDeletion, oana, Slice());
    m->Get(lkey, &val, &s);
    ASSERT_TRUE(s.IsNotFound());
...
TEST(MembufferTest, EmptyIterator) {
    MemBuffer* m = new MemBuffer();
    MemBufferIterator* iter = m->NewIterator();
    ASSERT_TRUE(!iter->Valid());
    delete iter;
    delete m;
}

TEST(MembufferTest, EmptyIteratorSeekToFirstInvalid) {
    MemBuffer* m = new MemBuffer();
...
TEST(MembufferTest, NonEmptyIteratorSeekToFirstValid) {
    MemBuffer* m = new MemBuffer();
...
TEST(MembufferTest, IteratorReturnValue) {
    MemBuffer* m = new MemBuffer();
    ...
    MemBufferIterator* iter = m->NewIterator();
    iter->SeekToFirst();
    ASSERT_EQ("OANA", *(IntToString(iter->value())));
    delete iter;
    delete m;
}

TEST(MembufferTest, IterateTest) {
    MemBuffer* m = new MemBuffer();
    ...
    Slice lol("LOL");
    m->Add(kTypeValue, oana, oana);
    m->Add(kTypeValue, igor, igor);
    m->Add(kTypeValue, lol, lol);


    MemBufferIterator* iter = m->NewIterator();
    for (iter->SeekToFirst(); iter->Valid(); iter->Next()) {
        std::cout << IntToString(iter->value()) << std::endl;        
    }
...
TEST(MembufferTest, Random) {
    MemBuffer* m = new MemBuffer();


    for (uint64_t i = 1; i <= 1<<20; i++) {
        m->Add(kTypeValue, IntToSlice(i), IntToSlice(i));
    }


    MemBufferIterator* iter = (MemBufferIterator*) m->NewIterator();
    uint64_t found_count = 0;
    for (uint64_t j = 1; j <= 2<<20; j++) {
        iter->RandomEntry();
        if (iter->AtValidKey()) {
            found_count++;
        }
...
void* IgorMTThreadBody(void* arg) {
  ...
  if (id == 0) {
    for (j = leveldb::kRange - 1; j >= leveldb::kRange/2; j--) {
      ...
      mem->Add(kTypeValue, IntToSlice(j), IntToSlice(j));
    }
  }
#else
  int i;
  for(i = 0; i < num_elems_thread; i++) {
    ...
    if (!mem->Get(LookupKey(IntToSlice(key), 0), &result, &s) || s.IsNotFound()) {
      mem->Add(kTypeValue, IntToSlice(key), IntToSlice(key));
    } else {
      i--;
    }
  }
  ...
  while (stop == 0) {
    ...
    if (unlikely(c <= scale_put)) {                 
      ...
      START_TS(2);              
      mem->Add(kTypeDeletion, IntToSlice(key), Slice());
      if(s.ok()) {               
...
TEST(MembufferTest, IgorMultiThreaded) {
